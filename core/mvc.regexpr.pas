...
{ off $DEFINE UseWordChars}
// Use WordChars property, otherwise fixed list 'a'..'z','A'..'Z','0'..'9','_' 
...
// Exceptions will point to appropriate source line, not to Error procedure
{$ENDIF}
...
const
  ...
  RegExprHorzSeparators: RegExprString = #9#$20#$A0
    {$IFDEF UniCode} + #$1680#$2000#$2001#$2002#$2003#$2004#$2005#$2006#$2007#$2008#$2009#$200A#$202F#$205F#$3000
    {$ENDIF};
...
type
  ...
  TRegExpr = class
  private
    GrpStart: array [0 .. RegexMaxGroups - 1] of PRegExprChar;
    // pointer to group start in InputString
    GrpEnd: array [0 .. RegexMaxGroups - 1] of PRegExprChar;
    // pointer to group end in InputString

    GrpIndexes: array [0 .. RegexMaxGroups - 1] of integer;
    // map global group index to _capturing_ group index
    GrpNames: array [0 .. RegexMaxGroups - 1] of RegExprString;
    // names of groups, if non-empty
    GrpAtomic: array [0 .. RegexMaxGroups - 1] of boolean;
    // group[i] is atomic (filled in Compile)
    GrpAtomicDone: array [0 .. RegexMaxGroups - 1] of boolean;
    // atomic group[i] is "done" (used in Exec* only)
    GrpOpCodes: array [0 .. RegexMaxGroups - 1] of PRegExprChar;
    // pointer to opcode of group[i] (used by OP_SUBCALL*)
    GrpSubCalled: array [0 .. RegexMaxGroups - 1] of boolean;
    // group[i] is called by OP_SUBCALL*
    GrpCount: integer;
    ...
    // string which must occur in match (got from regMust/regMustLen)
    ...
    // some attempt to prevent 'catastrophic backtracking' but not used
    ...
    // pointer to last emitted opcode; changing in pass-2, but =@regDummy in pass-1
    ...
    // pointer to length of substring of OP_EXACTLY* inside opcode
    ...
    function FindInCharClass(ABuffer: PRegExprChar; AChar: REChar;
      AIgnoreCase: boolean): boolean;
    procedure GetCharSetFromCharClass(ABuffer: PRegExprChar;
      AIgnoreCase: boolean; var ARes: TRegExprCharset);
 ...
 {$IFDEF InlineFuncs}inline;{$ENDIF}
    ...
    function IsCustomLineSeparator(AChar: REChar): boolean;
 {$IFDEF InlineFuncs}inline;{$ENDIF}
    {$IFDEF UseLineSep}
    procedure InitLineSepArray;
    {$ENDIF}
    procedure FindGroupName(APtr: PRegExprChar; AEndChar: REChar;
      var AName: RegExprString);
    ...
    function MatchAtOnePos(APos: PRegExprChar): boolean;
 {$IFDEF InlineFuncs}inline;{$ENDIF}

    // Exec for stored InputString
    function ExecPrim(AOffset: integer;
...
const
  ...
  RegExprAllSet = [0 .. 255];
  RegExprWordSet = [Ord('a') .. Ord('z'), Ord('A') .. Ord('Z'), Ord('0')
    .. Ord('9'), Ord('_')];
...
function _FindCharInBuffer(SBegin, SEnd: PRegExprChar; Ch: REChar): PRegExprChar;
...
function IsIgnoredChar(AChar: REChar): boolean; {$IFDEF InlineFuncs}inline;
{$ENDIF}
begin
  case AChar of
    ' ', #9, #$d, #$a:
...
function _IsMetaChar(AChar: REChar): boolean; {$IFDEF InlineFuncs}inline;
{$ENDIF}
begin
  case AChar of
    'd', 'D',
...
function InvertCase(const Ch: REChar): REChar; {$IFDEF InlineFuncs}inline;
{$ENDIF}
begin
...
begin
  Result :=
    (A.I = B.I) and (A.G = B.G) and (A.M = B.M) and (A.S = B.S) and
    (A.R = B.R) and (A.X = B.X);
end;

function ParseModifiers(const APtr: PRegExprChar; ALen: integer;
...
function _IsMetaSymbol1(ch: REChar): boolean; {$IFDEF InlineFuncs}inline;
{$ENDIF}
begin
  case ch of
    '^', '$', '.', '[', '(', ')', '|', '?', '+', '*', EscChar, '{':
...
function _IsMetaSymbol2(ch: REChar): boolean; {$IFDEF InlineFuncs}inline;
{$ENDIF}
begin
  case ch of
    '^', '$', '.', '[', '(', ')', '|', '?', '+', '*', EscChar, '{',
...
begin
  ...
  try
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              ASubExprs[SubExprIdx] :=
                System.Copy(ARegExpr, StartPos + 1, SubExprLen - 2);
        ...
        '#':
          ...
          begin
            ...
            while (i <= Len) and (ARegExpr[i] <> #$d) and (ARegExpr[i] <> #$a)
              ...
              do
              Inc(i);
            while (i + 1 <= Len) and ((ARegExpr[i + 1] = #$d) or
                (ARegExpr[i + 1] = #$a)) do
    ...
    if (ASubExprs.Count = 0) or ((PtrInt(ASubExprs.Objects[0]) and $FFFF) <> 1) or
      (((PtrInt(ASubExprs.Objects[0]) shr 16) and $FFFF) <> Len)
...
begin
  case AErrorID of
    ...
    reeComplexBracesNotImplemented:
      Result :=
        'TRegExpr compile: if you use braces {} and non-greedy ops *?, +?, ?? for complex cases, enable {$DEFINE ComplexBraces}';
    reeUnrecognizedModifier:
      Result := 'TRegExpr compile: incorrect modifier in (?...)';
    reeBadLinePairedSeparator:
      Result :=
        'TRegExpr compile: LinePairedSeparator must countain two different chars or be empty';
...
function IsDigitChar(AChar: REChar): boolean; {$IFDEF InlineFuncs}inline;
{$ENDIF}
begin
  case AChar of
    '0' .. '9':
...
function IsHorzSeparator(AChar: REChar): boolean; {$IFDEF InlineFuncs}inline;
{$ENDIF}
begin
...
function IsVertLineSeparator(AChar: REChar): boolean; {$IFDEF InlineFuncs}inline;
{$ENDIF}
begin
  case AChar of
    #$d, #$a, #$b, #$c:
...
function FindSkippedMetaLen(PStart, PEnd: PRegExprChar): integer;
 {$IFDEF InlineFuncs}inline;
{$ENDIF}
...
function TRegExpr.FindInCharClass(ABuffer: PRegExprChar; AChar: REChar;
  AIgnoreCase: boolean): boolean;
...
procedure TRegExpr.GetCharSetFromCharClass(ABuffer: PRegExprChar;
  AIgnoreCase: boolean; var ARes: TRegExprCharset);
var
  ch, ch2: REChar;
...
var
  ...
  procedure EmitSimpleBraces(ABracesMin, ABracesMax: TREBracesArg;
    ANonGreedyOp, APossessive: boolean);
  begin
    if APossessive then
  ...
  case (regParse - 1)^ of
    ...
    begin
      if not fCompModifiers.M
        {$IFDEF UseLineSep} or (fLineSeparators = '') {$ENDIF}
      then
        ret := EmitNode(OP_BOL)
      else
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if fCompModifiers.R and (RangeBeg = RusRangeLoLow) and
            (RangeEnd = RusRangeHiHigh) then
          begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              DashForRange :=
                (regParse + 2 < fRegexEnd) and ((regParse + 1)^ = '-') and
                ((regParse + 2)^ <> ']');
    ...
    begin
      ...
      case GrpKind of
        ...
        begin
          ...
          while (regParse < fRegexEnd) and (regParse^ <> ')') do
            Inc(regParse);
          if (regParse^ <> ')') or not ParseModifiers(SavedPtr,
            regParse - SavedPtr, fCompModifiers) then
          begin
    ...
    begin
      ...
      begin
        ...
        if (Len > 1) and ((EnderChar = '*') or (EnderChar = '+') or
          (EnderChar = '?') or (EnderChar = '{')) then
...
procedure TRegExpr.FindGroupName(APtr: PRegExprChar; AEndChar: REChar;
  var AName: RegExprString);
  ...
  case PREOp(p)^ of
    ...
    begin // ###0.936
      ...
      repeat
        ...
        while opnd < CurEnd do
        begin
          if (scan >= fInputEnd) or ((scan^ <> opnd^) and
            (scan^ <> InvertCase(opnd^))) then
            Exit;
          Inc(scan);
          Inc(opnd);
        end;
    ...
    case scan^ of
      ...
      begin
        ...
        begin
          ...
          if (regInput = fInputEnd - 2) and (regInput^ = #13) and
            ((regInput + 1)^ = #10) then
          begin
          end
          else
      ...
      OP_BOLML:
        if regInput > fInputStart then
        begin
          if ((regInput - 1) <= fInputStart) or not
            IsPairedBreak(regInput - 2) then
          begin
      ...
      OP_ANYML:
      begin
        if (regInput >= fInputEnd) or IsPairedBreak(regInput) or
          IsCustomLineSeparator(regInput^) then
          Exit;
          {$IFDEF UNICODEEX}
          IncUnicode(regInput);
          {$ELSE}
      ...
      begin // ###0.936
        ...
        while opnd < GrpEnd[no] do
        begin
          if (save >= fInputEnd) or ((save^ <> opnd^) and
            (save^ <> InvertCase(opnd^))) then
            Exit;
          Inc(save);
          Inc(opnd);
        end;
      ...
      OP_ANYOF:
      begin
        if (regInput >= fInputEnd) or not FindInCharClass(scan +
          REOpSz + RENextOffSz, regInput^, False) then
          Exit;
          {$IFDEF UNICODEEX}
          IncUnicode(regInput);
          {$ELSE}
      ...
      OP_ANYBUT:
      begin
        if (regInput >= fInputEnd) or FindInCharClass(scan + REOpSz +
          RENextOffSz, regInput^, False) then
          Exit;
          {$IFDEF UNICODEEX}
          IncUnicode(regInput);
          {$ELSE}
      ...
      begin
        if (regInput >= fInputEnd) or not FindInCharClass(scan +
          REOpSz + RENextOffSz, regInput^, True) then
          Exit;
          {$IFDEF UNICODEEX}
          IncUnicode(regInput);
          {$ELSE}
      ...
      begin
        if (regInput >= fInputEnd) or FindInCharClass(scan + REOpSz +
          RENextOffSz, regInput^, True) then
          Exit;
          {$IFDEF UNICODEEX}
          IncUnicode(regInput);
          {$ELSE}
      ...
      begin // ###0.940
        ...
        opnd := scan + PRENextOff(AlignToPtr(scan + REOpSz +
          RENextOffSz + 2 * REBracesArgSz))^;
        BracesMin := PREBracesArg(AlignToInt(scan + REOpSz + RENextOffSz))^;
        BracesMax := PREBracesArg(AlignToPtr(scan + REOpSz +
          RENextOffSz + REBracesArgSz))^;
        save := regInput;
...
function TRegExpr.ExecNext(ABackward: boolean {$IFDEF DefParam} =
  False{$ENDIF}): boolean;
var
...
function PrintableChar(AChar: REChar): RegExprString; {$IFDEF InlineFuncs}inline;
{$ENDIF}
begin
  if AChar < ' ' then
...
begin
  ...
  begin // While that wasn't END last time...
    ...
    if (op = OP_ANYOF) or (op = OP_ANYOFCI) or (op = OP_ANYBUT) or
      (op = OP_ANYBUTCI) then
    begin
      repeat
        case s^ of

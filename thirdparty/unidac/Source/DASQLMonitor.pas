...
{ TCustomDASQLMonitor }
  ...
  { DBMonitor support }
    ...
    procedure InternalSQLPrepare(Obj: TObject; const SQL: string; Params: TDAParams; BeforeEvent: boolean; var MessageID: Cardinal);
    procedure InternalSQLUnprepare(Obj: TObject; const SQL: string; Params: TDAParams; BeforeEvent: boolean; var MessageID: Cardinal);
    procedure InternalSQLExecute(Obj: TObject; const SQL: string; Params: TDAParams; const Caption: string; BeforeEvent: boolean; var MessageID: Cardinal); virtual;
    ...
    procedure InternalTRSavepoint(Obj: TObject; const Savepoint: string; BeforeEvent: boolean; var MessageID: Cardinal);
    procedure InternalTRRollbackToSavepoint(Obj: TObject; const Savepoint: string; BeforeEvent: boolean; var MessageID: Cardinal);
    procedure InternalTRReleaseSavepoint(Obj: TObject; const Savepoint: string; BeforeEvent: boolean; var MessageID: Cardinal);
    ...
    procedure InternalCustomMessage(Obj: TObject; const Msg: string);
    procedure InternalObjectDestroyed(Obj: TObject);
    procedure InternalPoolMessage(Obj: TObject; const Msg: string; WithCount: boolean; WithParams: boolean);

    class function GetMonitor: TCustomDASQLMonitor; virtual;
    procedure SetMonitor;virtual; abstract;

    function IsTransactionIDSupported(Obj: TObject): boolean; virtual;

  public
    class procedure SQLPrepare(Obj: TObject; const SQL: string; Params: TDAParams; var MessageID: Cardinal; BeforeEvent: boolean);
    class procedure SQLUnprepare(Obj: TObject; const SQL: string; Params: TDAParams; var MessageID: Cardinal; BeforeEvent: boolean);
    class procedure SQLExecute(Obj: TObject; const SQL: string; Params: TDAParams; const Caption: string; var MessageID: Cardinal; BeforeEvent: boolean);
    ...
    class procedure TRSavepoint(Obj: TObject; const Savepoint: string; var MessageID: Cardinal; BeforeEvent: boolean);
    class procedure TRRollbackToSavepoint(Obj: TObject; const Savepoint: string; var MessageID: Cardinal; BeforeEvent: boolean);
    class procedure TRReleaseSavepoint(Obj: TObject; const Savepoint: string; var MessageID: Cardinal; BeforeEvent: boolean);
    ...
    class procedure CustomMessage(Obj: TObject; const Msg: string);
    class procedure ObjectDestroyed(Obj: TObject);
    class procedure PoolMessage(Obj: TObject; const Msg: string; WithCount: boolean; WithParams: boolean = False);
    ...
    class function GetParams(Params: TDAParams; out SQLParams: TSQLParams): string; overload;
    class function GetParams(Params: TDAParams): string; overload;
    class function GetCaption: string; virtual;
    class procedure ShowDebug(Obj: TObject; const SQL: string; Params: TDAParams; const Caption: string);
...
{$IFDEF MSWINDOWS}
  ActiveX,
{$ENDIF}
  CRFunctions, CRConnectionPool, MemUtils {$IFNDEF FPC}, MemDS{$ENDIF};
...
class procedure TCustomDASQLMonitor.TRSavepoint(Obj: TObject; const Savepoint: string; var MessageID: Cardinal; BeforeEvent: boolean);
var
  Monitor: TCustomDASQLMonitor;
begin
  Monitor := GetMonitor;
...
class procedure TCustomDASQLMonitor.TRRollbackToSavepoint(Obj: TObject; const Savepoint: string; var MessageID: Cardinal; BeforeEvent: boolean);
var
  Monitor: TCustomDASQLMonitor;
begin
  Monitor := GetMonitor;
...
class procedure TCustomDASQLMonitor.TRReleaseSavepoint(Obj: TObject; const Savepoint: string; var MessageID: Cardinal; BeforeEvent: boolean);
var
  Monitor: TCustomDASQLMonitor;
begin
  Monitor := GetMonitor;
...
class procedure TCustomDASQLMonitor.CustomMessage(Obj: TObject; const Msg: string);
var
  Monitor: TCustomDASQLMonitor;
begin
  Monitor := GetMonitor;
...
class procedure TCustomDASQLMonitor.PoolMessage(Obj: TObject; const Msg: string;
...
{$ENDIF}
  ...
  if (Param.ValueCount <= 1) and ((Param.DataType in [ftString, ftFixedChar, ftWideString]) or
...
begin
  ...
  if Param.IsNull then
    Result := '<NULL>'
  else if Param.ValueCount > 1 then
    Result := '<ARRAY[' + IntToStr(Param.ValueCount) + ']>'
  else
    case Param.DataType of
      ...
      ftBlob, ftOraBlob, ftOraClob, ftMemo
...
var
  DataType, ParamType, Value: string;
begin
  DataType := GetParamDataType(Param);
  ParamType := GetParamParamType(Param);
  Value := GetParamValue(Param);

  SQLParam.Name := WideString(Param.Name);
  SQLParam.DataType := WideString(DataType);
  SQLParam.ParamType := WideString(ParamType);
  SQLParam.Value := WideString(Value);

  Result := ':' + Param.Name + '(' + ConcatWith([DataType,
    ParamType],',') + ConcatWith([')', Value], '=');
end;

class function TCustomDASQLMonitor.GetParams(Params: TDAParams; out SQLParams: TSQLParams): string;
var
  i: integer;
begin
  Result := '';
...
procedure TCustomDASQLMonitor.InternalSQLPrepare(Obj: TObject; const SQL: string; Params: TDAParams; BeforeEvent: boolean; var MessageID: Cardinal);
var
  ParamSt: string;
  SQLParams: TSQLParams;
  ASQL, Description: string;
begin
  CheckActive;
...
procedure TCustomDASQLMonitor.InternalSQLUnprepare(Obj: TObject; const SQL: string; Params: TDAParams; BeforeEvent: boolean; var MessageID: Cardinal);
var
  ParamSt: string;
  SQLParams: TSQLParams;
  ASQL, Description: string;
...
procedure TCustomDASQLMonitor.InternalSQLExecute(Obj:TObject; const SQL: string; Params: TDAParams; const Caption: string; BeforeEvent: boolean; var MessageID: Cardinal);
var
  ParamSt: string;
  ACaption: string;
  SQLParams: TSQLParams;
  ...
  if Active and (tfTransact in TraceFlags) then begin
    ...
    if moDBMonitor in Options then begin
      SendDBMonitorEvent(BeforeEvent, ET_BEGIN_TRANS, St, Obj, '',
         nil, False, '', MessageID);
      if IsTransactionIDSupported(Obj) then
        if not BeforeEvent then begin
          St :=  'Transaction ID = ' + IntToStr(TDBAccessUtils.GetMultiTransactionID(TDATransaction(Obj))) + ' ';
          SendDBMonitorEvent(True, ET_COMMIT, St, Obj, '',
            nil, False, '', MessageID);
          SendDBMonitorEvent(False, ET_COMMIT, St, Obj, '',
            nil, False, '', MessageID);
        end;
    end;
...
begin
  ...
  if Active and (tfTransact in TraceFlags) then begin
    ...
    if Obj is TCustomDAConnection then
      St := St + TCustomDAConnection(Obj).Username + '@' + TCustomDAConnection(Obj).Server
    else if IsTransactionIDSupported(Obj) then
      St :=  St + 'Transaction ID = ' + IntToStr(TDBAccessUtils.GetMultiTransactionID(TDATransaction(Obj))) + ' ';
    St := St + GetObjectHandle(Obj);
...
procedure TCustomDASQLMonitor.InternalTRSavepoint(Obj: TObject; const Savepoint: string; BeforeEvent: boolean; var MessageID: Cardinal);
var
  St: string;
begin
  CheckActive;
...
procedure TCustomDASQLMonitor.InternalTRRollbackToSavepoint(Obj: TObject; const Savepoint: string; BeforeEvent: boolean; var MessageID: Cardinal);
var
  St: string;
begin
  CheckActive;
...
procedure TCustomDASQLMonitor.InternalTRReleaseSavepoint(Obj: TObject; const Savepoint: string; BeforeEvent: boolean; var MessageID: Cardinal);
var
  St: string;
begin
  CheckActive;
...
procedure TCustomDASQLMonitor.InternalCustomMessage(Obj: TObject; const Msg: string);
var
  MessageID: cardinal;
  Description: string;
begin
...
procedure TCustomDASQLMonitor.InternalPoolMessage(Obj: TObject; const Msg: string;
...
begin
  ...
  if Active and (tfPool in TraceFlags) then begin
    Description := Msg;
    if WithCount then
      Description := Description + Format('. Pool has %d connection(s)',
...
function TCustomDASQLMonitor.IsTransactionIDSupported(Obj: TObject): boolean;
begin
  Result := False;
  if (Obj is TDATransaction) and (TDATransaction(Obj).DefaultConnection <> nil) then
    Result := TDBAccessUtils.IsMultipleTransactionsSupported(TDATransaction(Obj).DefaultConnection);
end;
...
begin
{$IFDEF MSWINDOWS}
  if not IsLibrary and not FRegistered and (GetMonitor = Self) then
...
begin
  Result := (csDesigning in ComponentState) or
            ((Owner <> nil) and not (csReading in Owner.ComponentState));
end;
...
begin
  FDBMonitor.Caption := WideString(GetCaption);
...
begin
  ...
  if BeforeEvent and (Obj <> nil) then begin
    ...
    Msg.ObjectName := WideString(GetObjectName(Obj));
    Msg.ObjectType := GetObjectType(Obj);
    Msg.ObjectTypeName := WideString(Obj.ClassName);

    if EventType <> ET_OBJ_DESTROY then begin
      Parent := GetParent(Obj);
      if Parent <> nil then begin
        Msg.ParentID := GetObjectID(Parent);
        Msg.ParentName := WideString(GetObjectName(Parent));
        Msg.ParentType := GetObjectType(Parent);
        Msg.ParentTypeName := WideString(Parent.ClassName);
      end;
    end;
  end;
  Msg.Description := WideString(Description);
  Msg.SQL := WideString(SQL);
  Msg.Params := Params;
  Msg.Failed := Failed;
  Msg.ErrorText := WideString(ErrorText);

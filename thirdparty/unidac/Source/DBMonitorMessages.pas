{$IFNDEF CLR}
{$IFNDEF DBMONITOR_HEADER}

{$I Dac.inc}
unit DBMonitorMessages;
{$DEFINE DBMONITOR_HEADER}
...
begin
  ...
  FVioTcp := TCRVioTcp.Create(FHost, FPort);
end;
...
begin
  ...
  if FLastConnectFailed and (GetTickInterval(FLastConnectTime, GetTickCount) < FReconnectTimeout) then
    exit;
  ...
  try
    if FVioTcp.TryConnect then begin
      FVioTcp.SendTimeout := FSendTimeout div 1000;
...
var
  cnt: Integer;
begin
  cnt := FVioTcp.Write({$IFNDEF CLR}TValueArr{$ENDIF}(FBuffer), 0, FBufferOffset);
  if cnt <> FBufferOffset then
    raise SocketException.Create(FVioTcp.LastError);
  FBufferOffset := 0;
end;
...
var
  ...
  offset, cnt: Integer;
begin
  if {$IFDEF CLR}Value <> nil{$ELSE}Value <> ''{$ENDIF} then begin
    Utf8Str := Encoding.UTF8.{$IFNDEF VER130}GetBytes{$ELSE}GetBytesWide{$ENDIF}(Value);
    Len := Length(Utf8Str);
  end
  else begin
    Utf8Str := nil;

...
type
  ...
  public
    ...
    constructor Create(ErrorCode: integer; Msg: string; IsServerError: boolean); overload;
    constructor Create(ErrorCode: integer; const Args: array of const; Msg: string); overload;

    property IsServerError: Boolean read FIsServerError;
...
constructor EMySqlException.Create(ErrorCode: integer; Msg: string; IsServerError: boolean);
begin
  FIsServerError := IsServerError;
  Create(ErrorCode, [], Msg);
end;
...
begin
  ...
  if Msg <> '' then
    if (ErrorCode = CR_CONN_HOST_ERROR) or (s = '') then
      s := Msg
    else
      s := s + #$D#$A + Msg;

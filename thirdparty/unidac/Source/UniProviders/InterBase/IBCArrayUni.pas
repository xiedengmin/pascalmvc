...
type
  ...
  public
    ...
    constructor Create(DbHandle: TISC_DB_HANDLE; TrHandle: TISC_TR_HANDLE; const TableName, ColumnName: string); overload;
    constructor Create(Connection: TGDSConnection; Transaction: TGDSTransaction; NeedFree: boolean = False); overload;
    constructor Create(Connection: TGDSConnection; Transaction: TGDSTransaction; const TableName, ColumnName: string); overload;
...
begin
  if FArrayDescType = adGDS then
    ...
    Offset := 8;
  Result := string(Marshal.PtrToStringAnsi(IntPtr(Integer({$IFNDEF FPC}FDesc{$ELSE}(@FDesc)^{$ENDIF}) + Offset)));
end;
...
constructor TCustomIBCArray.Create(DbHandle: TISC_DB_HANDLE; TrHandle: TISC_TR_HANDLE; const TableName, ColumnName: string);
begin
  inherited Create;
...
constructor TCustomIBCArray.Create(Connection: TGDSConnection; Transaction: TGDSTransaction; NeedFree: boolean);
begin
  inherited Create;
  ...
  if (FConnection <> nil) and (FTransaction <> nil) then
    Init(FConnection.GetDatabaseHandle, FTransaction.TransactionHandle)
  else
    Init(nil, nil);
end;

constructor TCustomIBCArray.Create(Connection: TGDSConnection; Transaction: TGDSTransaction; const TableName, ColumnName: string);
begin
  inherited Create;
...
begin
  if Value <> nil then
    SetTrHandle(Value.TransactionHandle);
  FTransaction := Value;
end;
...
begin
  if {$IFDEF CLR}integer{$ENDIF}(Status) > 0 then begin
    GDS.Busy;
    try
...
var
  ATableName,
  AColumnName: AnsiString;
  pTableName, pColumnName: IntPtr;
  Res: ISC_STATUS;
  i: integer;
begin
  FDescAccessor.Clear;
  ATableName := AnsiString(FTableName);
  pTableName := Marshal.StringToHGlobalAnsi(ATableName);
  AColumnName := AnsiString(FColumnName);
  pColumnName := Marshal.StringToHGlobalAnsi(AColumnName);
  try
    GDS.Busy;
    try
...
begin
  case FDescAccessor.DataType of
    ...
    blr_sql_date:
      DateTimeToIBCDate(TDateTime(Value), PtrOffset(FArrayBuffer, Offset));
    blr_sql_time:
      DateTimeToIBCTime(TDateTime(Value), PtrOffset(FArrayBuffer, Offset));
    blr_timestamp:
      DateTimeToIBCTimeStamp(TDateTime(Value), PtrOffset(FArrayBuffer, Offset));
...
begin
  ...
  if FInternalItemType = dtString then
    Result := WideString(GetItemAsString(Indices))
  else
    Result := '';
end;
...
var
  ...
  begin
    ...
    if Dimension = ArrayType.FDescAccessor.Dimensions then begin
      ...
      if (FConnection <> nil) and (FTransaction <> nil) then begin
        try
      ...
      TIBCArrayType(Attribute.ObjectType).FName := ArrayType.Name + '[' + IntToStr(ArrayType.AttributeCount) + ']';
...
begin
  ClearAttributes;
  ATableName := AnsiString(TableName);
  pTableName := Marshal.StringToHGlobalAnsi(ATableName);
  AColumnName := AnsiString(ColumnName);
  pColumnName := Marshal.StringToHGlobalAnsi(AColumnName);
  try
    GDS.Busy;
    try
...
procedure TCustomIBCArray.GetAttributeValue(const Name: string; out AttrBuf: IntPtr;
  out AttrLen: Word; out IsBlank, NativeBuffer: boolean);
var
...
begin
  ...
  if IsBlank and not (FInternalItemType in [dtObject, dtReference, dtArray]) then begin
    NativeBuffer := True;
    AttrBuf := nil;
    Exit;
  end;
  ...
  try
    ...
    if FDescAccessor.DataType in [blr_text, blr_text2, blr_cstring, blr_cstring2, blr_varying, blr_varying2] then begin
      Marshal.WriteByte(AttrBuf, FDescAccessor.Length, $00);
      AttrLen := Word(FDescAccessor.Length);
    end;
  except
...
procedure TCustomIBCArray.SetAttributeValue(const Name: string; ValuePtr: IntPtr; ValueLen: Word);
var
  Offset: integer;
  Ptr: IntPtr;
begin
  ...
  if FArrayBuffer = nil then
    AllocBuffer;
  Ptr := PtrOffset(FArrayBuffer, Offset);
  if Integer(ValueLen) <= Integer(FDescAccessor.Length) then
    CopyBuffer(ValuePtr, Ptr, ValueLen)
  else
    CopyBuffer(ValuePtr, Ptr, FDescAccessor.Length);
  FModified := True;
end;

function TCustomIBCArray.GetAttrIsNull(const Name: string): boolean;
begin
  Result := GetIsNull;
end;
  ...
  try
    ...
    repeat
      if (PrevCode = lcNumber) or (PrevCode = lcString) then begin
        if (Code <> lcSymbol) and (Code <> lxSemicolon) and (Code <> lxRightBracket) then

...
type
  ...
  protected
    ...
    procedure ExecuteStatement(const SQL: string; StatementType: integer; var Omit: Boolean; out BreakExec: boolean; Params: TDAParams = nil); override;
...
procedure TCustomIBCScriptProcessor.ExecuteStatement(const SQL: string; StatementType: integer;
...
procedure TCustomIBCScriptProcessor.DoBeforeStatementExecute(var SQL: string; StatementType: integer; var Omit: boolean);
var
  SQLParser: TParser;
  Code: integer;
  St, OldSt: string;
  DatabaseVal, UsernameVal, PasswordVal, RoleVal, ParamsVal: string;
  ...
  case StatementType of
    ...
    ST_CONNECT: begin
      ...
      if Code = lcString then
        DatabaseVal := AnsiDequotedStr(St, '''')
      else
        RaiseException;
      ...
      while True do begin
        ...
        if Code = lcString then
          St := AnsiDequotedStr(St, '''')
        else
          RaiseException;
        if SameText(OldSt, 'USER') then
          UsernameVal := St
        else
          if SameText(OldSt, 'PASSWORD') then
            PasswordVal := St
          else
            if SameText(OldSt, 'ROLE') then
              RoleVal := St;
      end;
...
procedure TCustomIBCScriptProcessor.DoAfterStatementExecute(var SQL: string; StatementType: integer);
begin

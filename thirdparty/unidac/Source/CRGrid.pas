...
{$ENDIF}
  CRFunctions, CRParser, CRAccess, MemDS, DBAccess;
...
begin
  if Assigned(Field) then
    Result := not (Field.DataType in [ftFmtMemo,
...
begin
  ...
  if Assigned(Field) then
    if (Field.DataSet is TCustomDADataSet) then begin
      if not (dgeLocalFilter in TCRDBGrid(Grid).OptionsEx) and
         (Field.FieldName <> TCRFieldDesc(TDBAccessUtils.GetIRecordSet(TCustomDADataSet(Field.DataSet)).Fields[Field.FieldNo - 1]).ActualName) then
        FieldName := TCRFieldDesc(TDBAccessUtils.GetIRecordSet(TCustomDADataSet(Field.DataSet)).Fields[Field.FieldNo - 1]).ActualName
      else
        FieldName := Field.FieldName;
      if not (dgeLocalFilter in TCRDBGrid(Grid).OptionsEx) and
         (TCRFieldDesc(TDBAccessUtils.GetIRecordSet(TCustomDADataSet(Field.DataSet)).Fields[Field.FieldNo - 1]).TableInfo.TableAlias <> '') then
        FieldName := TCRFieldDesc(TDBAccessUtils.GetIRecordSet(TCustomDADataSet(Field.DataSet)).Fields[Field.FieldNo - 1]).TableInfo.TableAlias + '.' + FieldName;
    end
    else
      FieldName := Field.FieldName;
    ...
    case Field.DataType of
      ...
      ftString,ftWideString: begin
        ...
        if (Field.DataSet is TCustomDADataSet) and (TCustomDADataSet(Field.DataSet).Options.QuoteNames) then
          Result := TDBAccessUtils.QuoteName(TCustomDADataSet(Field.DataSet), FieldName) + Sign +
            AnsiQuotedStr(ConstStr,'''')
        else
          if (Trim(ConstStr) = '''''') or (Trim(ConstStr) = '""') then
            Result := '(' + FieldName + Sign + 'null or ' + FieldName + Sign + ''''')'
          else
            Result := FieldName + Sign + AnsiQuotedStr(ConstStr, '''');
        Exit;
      end;
  ...
  Result := FieldName + Sign + ConstStr;
end;

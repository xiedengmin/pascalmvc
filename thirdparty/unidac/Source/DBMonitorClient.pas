...
begin
  if FStartupMessage = nil then begin
    ...
    with FStartupMessage do begin
      ...
      Description := WideString(Format(sAppStarted, [FCaption]));
      ExeName := WideString(ParamStr(0));
    {$IFDEF MSWINDOWS}
      if ExeName = '' then
        ExeName := WideString(GetModuleName);
    {$ENDIF}
    end;
  end;
...
begin
  ...
  try
    with Msg do begin
      ...
      Description := WideString(Format(sAppFinished, [FCaption]));
    end;
    AddEvent(Msg);
  except
    Msg.Free;
...
begin
  ...
  Resume;
  if IsLibrary then
    FConnectEvent.WaitFor(1000{INFINITE});
end;

destructor TEventSendThread.Destroy;
begin
  FConnectEvent.Free;
  FEndEvent.Free;
  inherited;
...
begin
  ...
  if IsLibrary then
    FEndEvent.WaitFor(1000{INFINITE});
end;

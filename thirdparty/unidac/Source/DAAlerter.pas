...
type
  ...
  TAlerterEventEvent = procedure (Sender: TDAAlerter; const EventName, Message: string) of object;
...
{ TDAAlerter }
  ...
  private
    ...
    procedure SetEvents(const Value: string);
  ...
  protected
    ...
    procedure DoOnEvent(const EventName, Message: string); virtual;
    ...
    property Events: string read GetEvents write SetEvents;
  ...
  public
    ...
    procedure SendEvent(const EventName, Message: string);
    procedure Start;
    procedure Stop;
...
begin
  try
    ...
    // ignore errors to avoid memory leaks
  end;
...
procedure TDAAlerter.SendEvent(const EventName, Message: string);
begin
  BeginConnection;
  try
...
var
  ...
  vUsedTransaction: TDATransaction;
begin
  if FInEndConnection then
    Exit;
  FInEndConnection := True;
...
procedure TDAAlerter.DoOnEvent(const EventName, Message: string);
begin
...
function TDAAlerter.GetEvents: string;
begin
  Result := DefaultSQLInfo.NamesFromList(FEvents, False);
end;

procedure TDAAlerter.SetEvents(const Value: string);
begin
  if Value <> Events then begin
    Stop;
    DefaultSQLInfo.NamesToList(Value, FEvents, False);

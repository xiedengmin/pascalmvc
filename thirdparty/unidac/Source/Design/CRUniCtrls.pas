...
{$IFDEF CPU64}
  PrevWin32Proc := Pointer(GetWindowLongPtr(FControl.Handle, GWL_WNDPROC));
{$ELSE}
  PrevWin32Proc := Pointer(GetWindowLong(FControl.Handle, GWL_WNDPROC));
{$ENDIF}
  PrevDefWin32Proc := FControl.DefWndProc;

  // subclass Window Procedures
{$IFDEF CPU64}
  SetWindowLongPtr(FControl.Handle, GWL_WNDPROC, NativeInt(ObjectInstance));
{$ELSE}
  SetWindowLong(FControl.Handle, GWL_WNDPROC, Integer(ObjectInstance));
{$ENDIF}
  FControl.DefWndProc := DefObjectInstance;
  SubClassWindowProc;

  // For some reason, caption gets garbled after calling SetWindowLong(.., GWL_WNDPROC).
...
{$IFDEF CPU64}
  SetWindowLongPtr(FControl.Handle, GWL_WNDPROC, NativeInt(PrevWin32Proc));
{$ELSE}
  SetWindowLong(FControl.Handle, GWL_WNDPROC, Integer(PrevWin32Proc));
{$ENDIF}

...
{$ENDIF}
  Classes,  SysUtils,
  DBAccess, CRAccess, DASQLGenerator,
  CRFrame, CRTabEditor;
...
begin
  ...
  SetControlEnabled(cbRecCount, IsEnabled and (Editor.LocalComponent is TCustomDADataSet));
  if not (Editor.LocalComponent is TCustomDADataSet) then
    cbRecCount.Checked := False;
...
begin
  ...
  Result := TDBAccessUtils.SQLGenerator(LocalDataSet).GenerateSQLforUpdTable(TDBAccessUtils.GetUpdater(LocalDataSet).ParamsInfo,
    TableInfo, KeyAndDataFields, _TStatementType(StatementType), ModifiedFieldsOnly);
end;
...
begin
  ...
  try
    ...
    DescribeDS := TComponentClass(OriginLocalDataSet.ClassType).Create(nil) as TCustomDADataSet;
    try
      DescribeDS.Assign(OriginLocalDataSet);
      TDBAccessUtils.SetDesigning(DescribeDS, csDesigning in OriginLocalDataSet.ComponentState);
      DescribeDS.FilterSQL := '1=0';
      DescribeDS.Debug := False;
      DescribeDS.Open;
      ...
      try
        ...
        for i := 0 to lbKeyFields.Items.Count - 1 do
          if lbKeyFields.Selected[i] then begin
            Field := DescribeDS.GetFieldDesc(lbKeyFields.Items[i], TableInfo.TableName) as TCRFieldDesc;
            if Field = nil then
              Field := LocalDataSet.GetFieldDesc(lbKeyFields.Items[i]) as TCRFieldDesc;

            KeyAndDataFields.KeyFieldDescs[j] := Field;
            Inc(j);
          end;
        ...
        for i := 0 to lbUpdateFields.Items.Count - 1 do
          if lbUpdateFields.Selected[i] then begin
            Field := DescribeDS.GetFieldDesc(lbUpdateFields.Items[i], TableInfo.TableName) as TCRFieldDesc;
            if Field = nil then
              Field := LocalDataSet.GetFieldDesc(lbUpdateFields.Items[i]) as TCRFieldDesc;
            KeyAndDataFields.DataFieldDescs[j] := Field;
            Inc(j);
          end;

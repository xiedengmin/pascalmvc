...
class function TUniDesignUtils.GetTableName(Obj: TCustomDADAtaSet): string;
begin
  Assert(Obj is TUniTable, Obj.ClassName);
...
class function TUniDesignUtils.GetStoredProcName(Obj: TCustomDADataSet): string;
begin
  Assert(Obj is TUniStoredProc, Obj.ClassName);
  Result := TUniStoredProc(Obj).StoredProcName;
end;

class procedure TUniDesignUtils.SetStoredProcName(Obj: TCustomDADataSet; const Value: string);
begin
  Assert(Obj is TUniStoredProc, Obj.ClassName);

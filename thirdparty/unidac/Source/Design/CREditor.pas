...
{$IFDEF MSWINDOWS}
  Windows, Messages,
{$ENDIF}
  SysUtils, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, ExtCtrls, Buttons,
  DBGrids,
{$IFDEF DBTOOLS}
  DBToolsClient,
{$ENDIF}
    ...
    for i := 0 to PropCount - 1 do begin
      ...
      if (PropInfo <> nil) // published property
        ...
        case PropList[i].PropType{$IFNDEF CLR}^{$ENDIF}.Kind of
          ...
          {$ELSE}{$IFDEF CPU64}
            Obj := TPersistent(Int64(GetPropValue(Control, string(PropList[i].Name))));
          {$ELSE}
          ...
          {$ENDIF}{$ENDIF}
            ...
            {$ELSE}{$IFDEF CPU64}
              NewObj := TPersistent(Int64(GetPropValue(NewControl, string(PropList[i].Name))));
            {$ELSE}
...
begin
{$IFDEF DBTOOLS}
  if Assigned(DBTools) and DBTools.HasDACSqlEditorFrame(Memo) then
...
begin
{$IFDEF DBTOOLS}
  if Assigned(DBTools) and DBTools.HasDACSqlEditorFrame(Memo) then begin
...
begin
  SL := nil;
{$IFDEF DBTOOLS}
  if not Assigned(DBTools) or (Assigned(DBTools) and not DBTools.HasDACSqlEditorFrame(Memo)) then
{$ENDIF}
    if Memo is TMemo then
      SL := TMemo(Memo).Lines
    else
...
{$IFDEF DBTOOLS}
  UseDBTools := Assigned(DBTools) and (SL = nil);
  if UseDBTools then
...
begin
  ...
  {$IFDEF USE_CODE_EDITOR}
    Items[9].Enabled := // IDE Editor
      not Boolean(GetPropValue(Memo, 'ReadOnly'));
  {$ENDIF}
  end;
end;
...
procedure TCREditorForm.Resize;
begin
  inherited;

  if imCorner <> nil then begin
    imCorner.Top := BtnPanel.Height - imCorner.Height;
    imCorner.Left := BtnPanel.Width - imCorner.Width;

    btCancel.Top := 8;
    btCancel.Left := BtnPanel.Width - btCancel.Width - 8;

    btOK.Top := 8;
    btOK.Left := BtnPanel.Width - btCancel.Width - 6 - btOK.Width - 8;
  end;
end;
...
var
  Control: TWinControl;
begin
  if Assigned(DBTools) and (Message.Active = WA_INACTIVE) then begin

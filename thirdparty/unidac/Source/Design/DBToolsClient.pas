...
{$IFDEF WIN32_64}
  ...
  VarUtils,
  {$ENDIF}
  {$IFDEF VER6P}DesignIntf,{$ELSE}DsgnIntf,{$ENDIF}
  ToolsApi,
{$ELSE}
...
{$IFNDEF CLR}
  DBToolsIntf,
{$ENDIF}
  {$IFDEF VER6P}ActnMan, {$ENDIF}ActnList, Graphics, Menus, Forms, Messages;
...
{$ENDIF}
  ...
  TDBToolsVerb = (dbtEditSql, dbtEditSelectSql, dbtQueryBuilder,
    dbtFindInDatabaseExplorer, dbtEditDatabaseObject, dbtExecuteSql, dbtDebugSql,
    dbtRetrieveData, dbtCompile, dbtCompileDebug,
    dbtEditInsertSql, dbtEditUpdateSql, dbtEditDeleteSql, dbtEditLockSql, dbtEditRefreshSql, dbtEditRecCountSQL);
  ...
  public
    ...
    procedure EditSql(AsQuery: boolean; StatementType: TStatementType = stQuery); virtual; abstract;
  ...
  protected
    ...
    procedure SetText(const Value: string); reintroduce; virtual; abstract;
  ...
  {$ENDIF}
    ...
    procedure EditSelectSql(Sender: TObject);
    procedure EditInsertSql(Sender: TObject);
    procedure EditUpdateSql(Sender: TObject);
    procedure EditDeleteSql(Sender: TObject);
    procedure EditRefreshSql(Sender: TObject);
    procedure EditLockSql(Sender: TObject);
    procedure EditRecCountSql(Sender: TObject);
  ...
  public
    ...
    function CreateDBToolsService(const DBToolsServiceClass: TCustomDBToolsServiceClass;
      ...
      const ClassID: TGUID; const DefaultConnectionStr: string; const ProviderKey: string;
      out ServiceVersion: int64; out NeedToCheck: TNeedToCheckDbTools): TObject;
    function CheckDevTools(const ClassID: TGUID; const ProviderKey: string;
      const DADesignUtils: TDADesignUtilsClass; out ServiceVersion: Int64): TNeedToCheckDbTools;
    ...
    property DesignNotification: TDBToolsDesignNotification read FDesignNotification write FDesignNotification;
  ...
  TModuleNotifier = class({$IFDEF CLR}TObject{$ELSE}TInterfacedObject, IOTANotifier, IOTAModuleNotifier{$IFDEF VER9P}, IOTAModuleNotifier80{$ENDIF}{$ENDIF})
  private
    FModule : IOTAModule;
  {$IFDEF WIN32_64}
...
{$ENDIF}
uses
  DADesign, Registry, Download, DAConsts, SysConst,
...
class function TCustomDBToolsService.GetNamespace: string;
begin
...
begin
  ...
  for i := 0 to SelectionList.Count - 1 do begin
    if SelectionList.Items[i] is TComponent then begin
      ...
      k := 0;
      for j := 0 to FSqlSourceList.Count - 1 do
        if TCustomSqlSource(FSqlSourceList[j]).FComponent = Component then begin
          TCustomSqlSource(FSqlSourceList[j]).CheckChanges;
          inc(k);
        end;
      if k = 0 then
...
function TDBTools.CheckDevTools(const ClassID: TGUID; const ProviderKey: string;
      const DADesignUtils: TDADesignUtilsClass; out ServiceVersion: Int64): TNeedToCheckDbTools;
...
begin
  ...
  try
    try
      ...
      if reg.OpenKeyReadOnly('CLSID\' + GUIDToString(ClassID) + '\InprocServer32') and
         reg.ValueExists('CodeBase') then begin
        Result := ncIncompatible;
        ServiceVersion := GetFileVersion(reg.ReadString('CodeBase'));
        reg.CloseKey;
...
procedure TDBTools.EditSelectSql(Sender: TObject);
begin
  if MenuItemsAvailable then
    FCurrentDBToolsService.EditSql(False, stQuery);
end;

procedure TDBTools.EditInsertSql(Sender: TObject);
begin
  if MenuItemsAvailable then
    FCurrentDBToolsService.EditSql(False, stInsert);
end;

procedure TDBTools.EditDeleteSql(Sender: TObject);
begin
  if MenuItemsAvailable then
    FCurrentDBToolsService.EditSql(False, stDelete);
end;

procedure TDBTools.EditUpdateSql(Sender: TObject);
begin
  if MenuItemsAvailable then
    FCurrentDBToolsService.EditSql(False, stUpdate);
end;

procedure TDBTools.EditRefreshSql(Sender: TObject);
begin
  if MenuItemsAvailable then
    FCurrentDBToolsService.EditSql(False, stRefresh);
end;

procedure TDBTools.EditLockSql(Sender: TObject);
begin
  if MenuItemsAvailable then
    FCurrentDBToolsService.EditSql(False, stLock);
end;

procedure TDBTools.EditRecCountSql(Sender: TObject);
begin
  if MenuItemsAvailable then
    FCurrentDBToolsService.EditSql(False, stRecCount);
end;
  ...
  for VerbIdx := Low(TDBToolsVerb) to High(TDBToolsVerb) do begin
    ...
    with FMenuActions[VerbIdx] do
      case VerbIdx of
        ...
        dbtEditSelectSql: begin
          Caption := 'Select';
          OnExecute := EditSelectSql;
        end;
        dbtEditInsertSql: begin
          Caption := 'Insert';
          OnExecute := EditInsertSql;
        end;
        dbtEditUpdateSql: begin
          Caption := 'Update';
          OnExecute := EditUpdateSql;
        end;
        dbtEditDeleteSql: begin
          Caption := 'Delete';
          OnExecute := EditDeleteSql;
        end;
        dbtEditLockSql: begin
          Caption := 'Lock';
          OnExecute := EditLockSql;
        end;
        dbtEditRefreshSql: begin
          Caption := 'Refresh';
          OnExecute := EditRefreshSql;
        end;
	dbtEditRecCountSQL: begin
          Caption := 'Rec Count';
          OnExecute := EditRecCountSql;
	end;
      end;
  {$IFDEF WIN32_64}
  {$IFDEF VER9P}

  ...
  with TDownloadForm.Create(Application) do
    try
      try
        ...
        if AtomName <> '' then
          if FindAtom({$IFDEF MSWINDOWS}PChar(AtomName){$ELSE}AtomName{$ENDIF}) = 0 then begin
            SetLength(AddedAtoms, Length(AddedAtoms) + 1);
            AddedAtoms[High(AddedAtoms)] := AddAtom({$IFDEF MSWINDOWS}PChar(AtomName){$ELSE}AtomName{$ENDIF});
          end
          else
            Exit;
      ...
      on E: Exception do
        MessageBox(0, {$IFDEF MSWINDOWS}PChar(E.Message){$ELSE}E.Message{$ENDIF},

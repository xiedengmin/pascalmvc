...
{$ENDIF}
  ...
  DBToolsIntf, DBToolsClient, Graphics, Menus, Forms,
...
type
  ...
  protected
    ...
    function GetSqlSource(Component: TComponent; Designer: IDesigner; SqlTextPrefix: string = ''; StatementType: TStatementType = stQuery): TSqlSource;
  {$IFDEF WIN32_64}
  ...
  public
    constructor Create(DBToolsService: TDBToolsService; Component: TComponent; Designer: IDesigner; StatementType: TStatementType = stQuery); reintroduce;
    destructor Destroy; override;
...
constructor TSqlSource.Create(DBToolsService: TDBToolsService; Component: TComponent; Designer: IDesigner; StatementType: TStatementType = stQuery);
var
  guid: TGUID;
begin
  inherited Create;
...
function TSqlSource.Get_ID: TString;
begin
...
procedure TDBToolsService.EditSql(AsQuery: boolean; StatementType: TStatementType = stQuery);
var
...
begin
  ...
  TSourceNotifier(SqlSource.SqlSourceNotifier).FSqlSourceNotifier := SqlSourceNotifier;   
...
function TDBToolsService.GetSqlSource(Component: TComponent; Designer: IDesigner; SqlTextPrefix: string = ''; StatementType: TStatementType = stQuery): TSqlSource;
begin
  Assert(DBTools.DesignNotification <> nil);
  Result := TSqlSource(DBTools.DesignNotification.SqlSourceList.Find(Component, SqlTextPrefix));
  if Result = nil then begin
    Result := TSqlSource.Create(Self, Component, Designer, StatementType);

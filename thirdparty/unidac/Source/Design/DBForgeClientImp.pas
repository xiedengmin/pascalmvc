...
{$ENDIF}
  ...
  Messages, CRFunctions, DASQLGenerator;
...
type
  ...
  protected
    ...
    function GetSqlSource(Component: TComponent; Designer: IDesigner; SqlTextPrefix: string = ''; StatementType: TStatementType = stQuery): TSqlSourceF;
  {$IFDEF WIN32_64}
  ...
  public
    constructor Create(DBToolsService: TDBForgeService; Component: TComponent; Designer: IDesigner; StatementType: TStatementType = stQuery); reintroduce;
    destructor Destroy; override;
...
constructor TSqlSourceF.Create(DBToolsService: TDBForgeService; Component: TComponent; Designer: IDesigner; StatementType: TStatementType = stQuery);
var
  guid: TGUID;
begin
  inherited Create;
...
function TSqlSourceF.Get_ID: TString;
begin
...
function TSqlSourceF.GetSqlText: string;
var
  SQL: TStrings;
  Macros: TMacros;
...
procedure TSqlSourceF.SetSqlText(Value: string);
var
  SQL: TStrings;
  Macros: TMacros;
  NewMacros: TDesignMacros;
...
function TDACSqlEditorFrameF.GetText: string;
begin
  if SqlEditor <> nil then
...
procedure TDACSqlEditorFrameF.SetText(const Value: string);
begin
  if SqlEditor <> nil then
...
procedure TDBForgeService.EditSql(AsQuery: boolean; StatementType: TStatementType = stQuery);
var
...
begin
  ...
  TSourceNotifierF(SqlSource.SqlSourceNotifier).FSqlSourceNotifier := SqlSourceNotifier;   
...
function TDBForgeService.GetSqlSource(Component: TComponent; Designer: IDesigner; SqlTextPrefix: string = ''; StatementType: TStatementType = stQuery): TSqlSourceF;
begin
  Assert(DBTools.DesignNotification <> nil);
  Result := TSqlSourceF(DBTools.DesignNotification.SqlSourceList.Find(Component, SqlTextPrefix));
  if Result = nil then begin
    Result := TSqlSourceF.Create(Self, Component, Designer, StatementType);

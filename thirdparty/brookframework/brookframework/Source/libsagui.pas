...
var
  ...
  sg_mm_set: function(malloc_func: sg_malloc_func; realloc_func: sg_realloc_func;
  free_func: sg_free_func): cint; cdecl;
...
var
  sg_httpres_sendbinary: function(res: Psg_httpres; buf: Pcvoid;
...
function sg_httpres_download(res: Psg_httpres; const filename: Pcchar;
  status: cuint): cint; {$IFNDEF DEBUG}inline;{$ENDIF}
...
var
  sg_httpres_zsendbinary2: function(res: Psg_httpres; level: cint;
  buf: Pcvoid; size: csize_t; const content_type: Pcchar; status: cuint): cint; cdecl;

  sg_httpres_zsendbinary: function(res: Psg_httpres; buf: Pcvoid;
  size: csize_t; const content_type: Pcchar; status: cuint): cint; cdecl;
...
var
  ...
  const filename: Pcchar; const disposition: Pcchar; status: cuint): cint; cdecl;
...
var
  ...
  sg_entrypoints_add: function(entrypoints: Psg_entrypoints;
  const path: Pcchar; user_data: Pcvoid): cint; cdecl;
...
var
  ...
  const pattern: Pcchar; errmsg: Pcchar; errlen: csize_t; cb: sg_route_cb;
  cls: Pcvoid): cint; cdecl;
...
var
  ...
  sg_expr_compile: function(expr: Psg_expr; const str: Pcchar;
  len: csize_t; extensions: Psg_expr_extension): cint; cdecl;
  ...
  sg_expr_set_var: function(expr: Psg_expr; const Name: Pcchar;
  len: csize_t; const val: cdouble): cint; cdecl;
...
type
  ...
  class var
    ...
    GHandle: TLibHandle;
  private
    class function InternalLoad(const AName: TFileName): TLibHandle; static;
  public
...
function sg_httpres_zdownload(res: Psg_httpres; const filename: Pcchar;
  status: cuint): cint;
begin
  Result := sg_httpres_zsendfile2(res, 1, 0, 0, 0, filename, 'attachment', status);
end;
...
constructor TSgLibUnloadHolder.Create(AEvent: TNotifyEvent; ASender: TObject);
begin
  inherited Create;
  FEvent := AEvent;
  FSender := ASender;
...
begin
  try
    ...
    if (((AVersion shr 16) and $FF) <> SG_VERSION_MAJOR) or
      ...
      raise EInvalidOpException.CreateFmt(SSgLibVersion,
...
begin //FI:C101
  ...
  try
    ...
    sg_httpsrv_set_post_buf_size :=
      GetProcAddress(GHandle, 'sg_httpsrv_set_post_buf_size');
    ...
    sg_httpsrv_set_thr_pool_size :=
      GetProcAddress(GHandle, 'sg_httpsrv_set_thr_pool_size');
    ...
    sg_entrypoint_set_user_data :=
      GetProcAddress(GHandle, 'sg_entrypoint_set_user_data');

...
type
  ...
  Sagui = record
    ...
    { Returns the library version number.
      ...
      @returns(Library version packed into a single integer.) }    class function Version(out AMajor, AMinor: byte; out APatch: smallint): cardinal; overload; static;
    { Returns the library version number as string in the
      format @code(<MAJOR>.<MINOR>.<PATCH>).
      @returns(Library version packed into a static string.) }    class function VersionStr: string; static;
    ...
    { Reallocates an existing memory block.
      ...
      @param(ASize[in] Memory size to be allocated.)
      @returns(Pointer of the reallocated memory.) }    class function Realloc(APointer: Pointer; ASize: nativeuint): Pointer; static;
    { Frees a memory space previous allocated by @code(Sagui.Malloc),
      @link(Sagui.Alloc) or @code(Sagui.Realloc).
      @param(APointer[in] Pointer of the memory to be freed.) }    class procedure Free(APointer: Pointer); static;
    { Returns string describing an error number.
      ...
      @param(AErrorLen[in] Length of the error message.) }    class procedure StrError(AErrorNum: integer; out AErrorMsg: string; AErrorLen: integer); overload; static; {$IFNDEF DEBUG}inline;{$ENDIF}
    { Returns string describing an error number.
      @param(AErrorNum[in] Error number.)
      @returns(Static string describing the error.) }    class function StrError(AErrorNum: integer): string; overload; static;
    { Checks if a string is an HTTP post method.
      @param(AMethod[in] HTTP verb.)
      @returns(True if given method is POST, PUT, DELETE or OPTIONS.) }    class function IsPost(const AMethod: string): boolean; static;
    { Extracts the entry-point of a path or resource. For example, given a path
      ...
      @returns(Entry-point as static string.) }    class function ExtractEntryPoint(const APath: string): string; static;
    ...
    { Indicates the end-of-read processed in
      ...
      @param(AError[in] @True to return a value indicating a stream
       reading error.)
      @returns(Value to end a stream reading.) }    class function EOR(AError: boolean): nativeint; static;
...
class function Sagui.Version(out AMajor, AMinor: byte; out APatch: smallint): cardinal;
begin
  SgLib.Check;
  Result := sg_version;
...
begin
  Result :=
{$IFDEF FPC}SHA1Print(SHA1String(S)){$ELSE}
...
function TBrookHTTPRequestMethodHelper.FromString(const AMethod: string):

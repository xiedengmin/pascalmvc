...
type
  ...
  protected
    class function DoStreamRead(Acls: Pcvoid; Aoffset: cuint64_t;
      Abuf: Pcchar; Asize: csize_t): cssize_t; cdecl; static;
  ...
  public
    ...
    { Sends a stream to the client.
      ...
      @param(AFreed[in] @True frees the stream automatically as soon as it
        is sent.)
      @param(AStatus[in] HTTP status code.) }
    procedure SendStream(AStream: TStream; AFreed: boolean; AStatus: word);
      overload; virtual;
    ...
    { Offers a file as download.
      @param(AFileName[in] Path of the file to be sent.)
      @param(AStatus[in] HTTP status code.) }
    procedure Download(const AFileName: TFileName; AStatus: word);
      overload; virtual;
...
procedure TBrookHTTPResponse.Send(const AValue, AContentType: string; AStatus: word);
var
  M: TMarshaller;
  R: cint;
begin
  ...
  if FCompressed then
  begin
    R := sg_httpres_zsendbinary(FHandle, M.ToCString(AValue),
...
procedure TBrookHTTPResponse.SendBytes(const ABytes: TBytes;
  ASize: nativeuint; const AContentType: string; AStatus: word);
var
  WorkAround: TBytes;
begin
...
procedure TBrookHTTPResponse.SendFile(ASize: nativeuint;
  AMaxSize, AOffset: uint64; const AFileName: TFileName; ADownloaded: boolean;
  AStatus: word);
var
  M: TMarshaller;
  R: cint;
...
procedure TBrookHTTPResponse.SendAndRedirect(
  const AValue, ADestination, AContentType: string; AStatus: word);
begin
...
procedure TBrookHTTPResponse.SendAndRedirect(
  const AValue, ADestination, AContentType: string);
begin
  SendAndRedirect(AValue, ADestination, AContentType, 302);
end;

procedure TBrookHTTPResponse.Download(const AFileName: TFileName; AStatus: word);
var
  M: TMarshaller;
  R: cint;
begin
...
procedure TBrookHTTPResponse.Render(const AFileName: TFileName; AStatus: word);
var
  M: TMarshaller;
  R: cint;
begin
